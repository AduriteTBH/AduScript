// Video Player & Watch Page Component in pure AduScript
import { formatViews, formatDuration, formatSubs } from "../utils/formatters.ads"
import { createCommentsSection } from "./comments.ads"

export fn createWatchPage(video, isPlayingState, likeState, isSubscribedState, commentsState, onBack, onAddComment, onSelectRelated, relatedVideos) {
  let viewsFormatted = formatViews(video.views)
  let subsFormatted = formatSubs(video.subscribers)
  let likesFormatted = (video.likes + (likeState.value ? 1 : 0)).toLocaleString()
  let subText = isSubscribedState.value ? "Subscribed ✓" : "Subscribe"
  let subClass = isSubscribedState.value ? "subscribed" : ""
  let likeClass = likeState.value ? "liked-active" : ""

  // Render related up-next videos
  let relatedItems = relatedVideos.filter(v -> v.id != video.id).slice(0, 6).map(v -> {
    return f`
      ...
      <div class="player-container">
          ...
          <div class="video-screen-inner" style="background:${video.colorGradient};">
            ...
            <div class="player-controls">
              ...
              <div class="progress-bar-container" onclick=${(e) -> {
                ...
                console.log(f"Seeked video to: {Math.round(pct * 100)}%")
              }}>
              ...
              <div class="player-actions-row">
                <div class="player-left-actions">
                  <button class="play-pause-btn interactive-btn" onclick=${() -> { isPlayingState.value = !isPlayingState.value }}>
        ...
        <div class="watch-meta-row">
          <div class="channel-subscribe-box">
            ...
            <button class="sub-btn subscribe-btn-anim ${subClass}" onclick=${() -> { isSubscribedState.value = !isSubscribedState.value }}>
              ${subText}
            </button>
          </div>
          ...
          <div style="display:flex;gap:10px;">
            ...
            <div class="action-pill-group">
              <button class="action-pill-btn ${likeClass}" onclick=${() -> { likeState.value = !likeState.value }}>
